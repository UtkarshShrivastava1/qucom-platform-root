...
interface CreateDebitNoteViewProps {
  onBack: () => void;
}

export const CreateDebitNoteView: React.FC<CreateDebitNoteViewProps> = ({ onBack }) => {
  const { createDebitNote, invoices } = useBillingStore();
  ...
  // Debit Note Details
  const [noteNo, setNoteNo] = useState('DN-1038');
  const [invoiceNo, setInvoiceNo] = useState('INV-1456');
  const [noteDate, setNoteDate] = useState('11 May 2024');
  const [reference, setReference] = useState('PO-4587');
  ...
  // Line items matching mockup 5.9.png
  ...
  const [terms, setTerms] = useState(
    `1. This debit note is issued against the original invoice mentioned above.\n2. The amount will be added to your next invoice as per agreed terms.\n3. This debit note is valid only for the items and quantity mentioned.\n4. Goods once sold will not be taken back unless agreed.`
  );
  ...
  const totalDebitAmount = Math.round((taxableAmount + totalTax) * 100) / 100;
  ...
  const handleAddItem = () => {
    ...
    const newItem: INoteItem = {
      ...
      name: defaultProd ? defaultProd.name : 'Shipping & Packaging Adjustment',
      hsnSac: '996511',
      description: 'Additional logistics fee',
      invoicedQty: 1,
      returnedQty: 1,
      quantity: 1,
      unit: 'Lot',
  ...
  const handleBarcodeScan = () => {
    ...
    setTimeout(() => {
      ...
      alert('Barcode scanned: Matched invoice INV-1456 line item.');
    }, 700);
  };
  ...
  const handleSave = (status: 'draft' | 'issued' = 'issued', print: boolean = false) => {
    createDebitNote({
      ...
      customerAddress,
      placeOfSupply,
      reason: 'Additional Charges / Adjustment',
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900">Create Debit Note</h2>
          <p className="text-xs text-slate-500 mt-0.5">
            Create a debit note to record additional charges for your customer.
          </p>
        </div>
      ...
      {/* 4 Cards Row: Customer, Address, Debit Note Details, Other Details */}
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        {/* Card 3: Debit Note Details */}
        <div className="bg-white rounded-2xl border border-slate-200/80 p-4 shadow-2xs space-y-2.5">
          <span className="block text-[11px] font-bold text-slate-700">Debit Note Details</span>
          <div>
            <label className="block text-[11px] font-semibold text-slate-600 mb-1">
              Debit Note No. <span className="text-rose-500">*</span>
            </label>
            <input
              type="text"
              value={noteNo}
              readOnly
              className="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-bold text-slate-600"
            />
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              >
                <option value="INV-1456">INV-1456</option>
                {invoices.map((i) => (
              ...
              <Search className="w-3.5 h-3.5 text-blue-600 absolute right-2.5 top-2.5 pointer-events-none" />
            </div>
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="Enter reference (e.g. PO No., etc.)"
              className="w-full px-2.5 py-1.5 bg-white border border-slate-300 rounded-xl text-xs font-medium text-slate-800 focus:outline-none focus:border-blue-500"
            />
          </div>
        </div>
      ...
      {/* Items Section (5.9.png) */}
      <div className="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-2xs space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-bold text-slate-900">
            Items (Select items to add debit)
          </h3>
          ...
          <table className="w-full text-left border-collapse min-w-[950px]">
            <thead>
              <tr className="bg-slate-50/80 border-b border-slate-200 text-[11px] font-semibold text-slate-600">
                ...
                <th className="py-2.5 px-2 text-center w-24">Quantity (Invoiced)</th>
      ...
      {/* Bottom Section: Terms & Conditions + Debit Summary (5.9.png) */}
        ...
        <div className="lg:col-span-5 bg-white rounded-2xl border border-slate-200/80 p-5 shadow-2xs space-y-3">
          ...
          <div className="pt-3 border-t border-slate-200">
            <div className="flex justify-between items-baseline">
              <span className="text-sm font-extrabold text-slate-900">Total Debit Amount</span>
